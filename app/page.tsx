...
type ScanResult = {
  ...
  dataLabel: string;
  limitations: string[];
  stats: { snapshots: number; discoveryPassed: number; deepValidated: number; finalists: number };
  ...
  return (
    <main>
      <header className="topbar">
        ...
        <div className="data-pill">TIINGO · CONSOLIDATED</div>
      </header>

      <section className="hero">
        <div>
          <p className="eyebrow">MOMENTUM ENGINE · V0.1</p>
          <h1>Find setups worth<br/><span>paying attention to.</span></h1>
          <p className="sub">Discovery first. Live validation second. AI only analyzes finalists. If nothing clears the bar, the scanner says NO TRADE.</p>
        </div>
        <button className="scanButton" onClick={scan} disabled={loading}>
          {loading ? <><span className="spinner"/> SCANNING MARKET</> : <>SCAN MARKET <span>↗</span></>}
        </button>
      </section>

      <section className="rules">
        <span>$1–$50</span><span>+5%+</span><span>1M+ VOL</span><span>RVOL</span><span>VWAP</span><span>HOD</span><span>LIQUIDITY</span><span>CATALYST</span>
      </section>

      {loading && <section className="loadingPanel"><div className="scannerLine"/><p>Filtering the market, validating intraday bars, checking news, then ranking finalists…</p></section>}
      {error && <section className="error"><strong>Scan failed:</strong> {error}<br/><small>Check TIINGO_API_KEY and OPENAI_API_KEY in your environment variables.</small></section>}
        ...
        <>
          <section className={`verdict ${result.status === "NO_TRADE" ? "noTrade" : "hasTrades"}`}>
            <div>
              ...
              <p>{result.status === "NO_TRADE" ? "No finalist met the complete quality threshold. Standards were not lowered." : "At least one finalist cleared the current momentum threshold. Broker confirmation is still required."}</p>
            </div>
            <div className="stats">
            ...
            {result.candidates.map((c, i) => (
              <article className="card" key={c.ticker}>
                ...
                <div className="scoreRow">
                  ...
                  <div><span>SPREAD</span><b>{pct(c.spreadPct, 2)}</b></div>
                </div>
                ...
                {c.news.length > 0 && <details><summary>Recent catalyst/news sources ({c.news.length})</summary><div className="newsList">{c.news.map((n, x) => <a key={x} href={n.url} target="_blank" rel="noreferrer"><b>{n.title}</b><span>{n.source} · {n.publishedDate ? new Date(n.publishedDate).toLocaleString() : ""}</span></a>)}</div></details>}
              </article>
            ))}
          </section>

          <section className="limitations"><strong>DATA / RISK NOTES</strong>{result.limitations.map((x, i) => <p key={i}>• {x}</p>)}<p>• Feed: {result.dataLabel}</p></section>
        </>
      )}

      <footer>Research scanner only · Not financial advice · Always confirm executable prices in your broker.</footer>
    </main>
  );
}
